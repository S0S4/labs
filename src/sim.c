...
// ADDIU
   ...
   uint32_t rt = (curr_ins & RT_MASK) >> 17;
   uint16_t imm = (curr_ins & IM_MASK);
    
    printf("RS: %x \n RT: %x \n IMM: %x \n", rs, rt, imm);

    uint32_t aux = imm + mem_read_32(CURRENT_STATE.REGS[rt]);
    printf("AUX %X\n", aux);
    printf("REG[ZERO] VALUE: %x\n", mem_read_32(CURRENT_STATE.REGS[rt]));
    printf("REG[ZERO]: %x\n", CURRENT_STATE.REGS[rt]);
    mem_write_32(CURRENT_STATE.REGS[rs],aux);
    printf("REG[RS]: %x\n", CURRENT_STATE.REGS[rs]);
/*
        for(uint16_t i = 0; i < MIPS_REGS; i++){
